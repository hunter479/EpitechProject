...
SpeedUp::SpeedUp(irr::IrrlichtDevice *device, vector3df pos, IMeshSceneNode *node)
{
	// IMesh *mesh = core.getSmgr()->getMesh("./media/powerUpSpeed.obj");
	// node = core.getSmgr()->addMeshSceneNode(mesh);
	if (node) {
        node->setMaterialTexture(0, device->getVideoDriver()->getTexture("./media/speedUpTexture.png"));
